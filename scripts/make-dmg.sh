...
1. 把 sugarnote.app 拖进左边的 Applications
   ⚠️ 一定要先拖到 Applications 再运行。直接从 DMG 里运行的话，
      macOS 会把它放到一个随机只读路径里执行，功能会不正常。
2. 打开它——但第一次会被 Gatekeeper 拦下（见下一节）
3. 它会弹窗要辅助功能权限，点「打开系统设置」，
   在「隐私与安全性 > 辅助功能」里把 sugarnote 打开

首次打开被系统拦下怎么办
------------------------
这个 App 没有 Apple 开发者账号签名、也没公证，所以系统会拦。
提示通常是「无法打开，因为 Apple 无法检查其是否包含恶意软件」。

放行办法（二选一）：

  方法一（推荐，图形界面）：
    先双击一次让它被拦（这一步是必须的，否则下个界面里不会出现按钮），
    然后打开「系统设置 > 隐私与安全性」，往下滚到「安全性」一节，
    会看到「已阻止使用 sugarnote.app，因为来自身份不明的开发者」，
    点右边的「仍要打开」，再确认一次。之后就能正常打开了。

  方法二（终端，最直接）：
    xattr -d com.apple.quarantine /Applications/sugarnote.app

这不是本 App 特有的问题：所有没有付费开发者账号的开源 macOS 应用都这样。
介意的话可以自己从源码构建（仓库里有完整步骤）。
