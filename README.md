...
sugarnote 只在备忘录打开时才有意义。没必要让它开机常驻，也不用每次手动开关——
用「快捷指令」的自动化把它和备忘录绑在一起，打开备忘录它就在，关掉备忘录它自己退。

打开「快捷指令」→ 侧边栏「自动化」→ 新建两条：

| 触发条件 | 动作 |
| --- | --- |
| 备忘录 **已打开** | 打开 App → sugarnote |
| 备忘录 **已关闭** | 退出 App → sugarnote |

两个要点：

- 触发条件在「App」分类下，选好 App 之后能选「已打开 / 已关闭」。
- 每条都要把「运行前询问」改成**立即运行**，否则每次开备忘录都会弹一下问你
  「要不要运行这个自动化」，跟没自动化一样。

这样还有个附带好处：sugarnote 的运行范围和备忘录完全重合。它不在后台待着，
也就不会在你不写笔记的时候有任何存在感。

> 如果你更喜欢它常驻，把 App 拖进「系统设置 > 通用 > 登录项」也行。
> 但既然它只服务于备忘录，跟备忘录同生共死更干净。
...
状态栏没有图标 = App 根本没在运行（用了上面的自动化的话，打开备忘录它应该自己起来；
否则手动启动一次）。有图标的话，菜单第一行就是当前状态
（「正在工作」/「需要辅助功能权限」/「备忘录未运行」）。
